...
<div class="space-y-4">
    <form method="GET" class="flex flex-wrap items-end gap-3">
        <div class="min-w-[min(100%,280px)] flex-1 sm:max-w-md">
            <label for="account" class="mb-1 block text-xs font-bold text-slate-600">Akun</label>
    ...
    <div class="grid gap-3 sm:grid-cols-3">
        <div class="rounded-2xl bg-white p-4 ring-1 ring-black/5">
            <p class="text-xs font-semibold text-slate-500">Saldo awal periode</p>
            <p class="mt-1 text-lg font-extrabold tabular-nums text-slate-800">{{ FormatHelper::glBalance($opening_balance) }}</p>
            <p class="mt-0.5 text-xs text-slate-400">Per {{ FormatHelper::dateId($from) }}</p>
        </div>
        <div class="rounded-2xl bg-white p-4 ring-1 ring-black/5">
            <p class="text-xs font-semibold text-slate-500">Mutasi dalam periode</p>
            <p class="mt-1 text-lg font-extrabold text-slate-800">{{ $txCount }} transaksi</p>
        ...
        <div class="rounded-2xl bg-amber-50 p-4 ring-1 ring-amber-200/60">
            <p class="text-xs font-semibold text-amber-800/80">Saldo akhir periode</p>
            <p class="mt-1 text-lg font-extrabold tabular-nums text-amber-900">{{ FormatHelper::glBalance($closing_balance) }}</p>
            <p class="mt-0.5 text-xs text-amber-700/70">Per {{ FormatHelper::dateId($to) }}</p>
        </div>
    </div>
    ...
    <div class="bakery-card overflow-hidden">
        <div class="border-b border-slate-100 bg-slate-50 px-4 py-3 sm:px-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                    <p class="text-xs font-bold uppercase tracking-wide text-slate-500">Buku Besar</p>
                    ...
                    <p class="mt-2 text-xs text-slate-500">
                        Menampilkan jurnal dari {{ FormatHelper::dateId($from) }} hingga {{ FormatHelper::dateId($to) }}.
                        Baris pertama adalah saldo sebelum tanggal awal.
                    </p>
                </div>
            ...
            <table class="bakery-table text-sm">
                <thead>
                    <tr>
                        ...
                        <th class="w-[96px]">Tanggal</th>
                        <th class="min-w-[140px]">Ref</th>
                        <th class="w-[128px] text-right">Debit</th>
                        <th class="w-[128px] text-right">Kredit</th>
                        <th class="w-[148px] text-right">Saldo</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($rows as $r)
                    ...
                    @if ($rows->isNotEmpty())
                        <tr class="bg-amber-50 font-extrabold text-slate-900">
                            <td colspan="3" class="text-right sm:text-left">Saldo akhir</td>
