...
@php
    ...
    $title = __('app.pages.dashboard').' - '.__('app.common.admin');
    $role = 'admin';
    $active = 'admin.dashboard';
    $pageTitle = __('app.pages.dashboard');
    $pageSubtitle = __('app.pages.dashboard_subtitle_admin', ['month' => $monthName ?? now()->format('F Y')]);
    ...
    $financeRows = [
        [
            'label' => __('dashboard.finance_sales'),
            'hint' => __('dashboard.finance_sales_hint'),
            'value' => $salesMonth,
            'bar' => 'bg-amber-500',
            'text' => 'text-amber-800',
        ...
        [
            'label' => __('dashboard.finance_gross'),
            'hint' => __('dashboard.finance_gross_hint'),
            'value' => $grossProfit,
            'bar' => 'bg-sky-500',
            'text' => 'text-sky-800',
        ...
        [
            'label' => __('dashboard.finance_net'),
            'hint' => __('dashboard.finance_net_hint'),
            'value' => $netProfit,
...
<div class="space-y-5 sm:space-y-6 bakery-page">
    ...
    <div class="grid grid-cols-1 gap-4 min-[420px]:grid-cols-2 xl:grid-cols-4">
        <x-kpi-card
            :title="__('dashboard.kpi_total_stock')"
            :value="(string) $stockCount"
            :sub="__('dashboard.kpi_below_minimum', ['count' => $lowStockCount])"
            tone="rose"
            :icon="$ic('M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4')"
        />
        <x-kpi-card
            :title="__('dashboard.kpi_success_production')"
            :value="(string) $successProduction"
            :sub="$totalProduction > 0 ? __('dashboard.kpi_failed_batches', ['failed' => $failedProduction, 'total' => $totalProduction]) : __('dashboard.kpi_no_production')"
            tone="green"
            :icon="$ic('M9 12l2 2 4-4m6 2a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z')"
        />
        <x-kpi-card
            :title="__('dashboard.kpi_operational_cost')"
            :value="FormatHelper::rupiah($operationalMonth)"
            :sub="__('dashboard.kpi_current_month')"
            tone="blue"
            :icon="$ic('M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8V6m0 12v-2')"
        />
        <x-kpi-card
            :title="__('dashboard.kpi_sales_30d')"
            :value="FormatHelper::rupiah($salesLast30)"
            :sub="__('dashboard.kpi_vs_prev_week')"
    ...
    <div class="grid gap-5 lg:grid-cols-3">
        <div class="bakery-card overflow-hidden lg:col-span-2">
            <div class="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-amber-50/80 to-white px-4 py-4 sm:px-5">
                <div>
                    <p class="text-xs font-bold uppercase tracking-wide text-amber-800/70">{{ __('dashboard.analytics') }}</p>
                    <h2 class="text-base font-extrabold text-slate-900">{{ __('dashboard.sales_trend') }}</h2>
                    <p class="mt-0.5 text-xs text-slate-500">{{ __('dashboard.sales_trend_sub', ['total' => FormatHelper::rupiah($salesChart['total'])]) }}</p>
                </div>
                @if ($hasSalesChart && $salesChart['peak'] > 0)
                    <div class="rounded-xl bg-white px-3 py-2 text-right ring-1 ring-amber-200/60">
                        <p class="text-[10px] font-bold uppercase text-amber-800/60">{{ __('dashboard.peak') }}</p>
                ...
                @if ($hasSalesChart)
                    <div class="relative h-[260px] w-full sm:h-[280px]">
                        <canvas
                            ...
                            aria-label="{{ __('dashboard.chart_sales_aria') }}"
                        ></canvas>
                    </div>
                @else
                    <div class="flex h-[220px] flex-col items-center justify-center rounded-2xl bg-slate-50 text-center ring-1 ring-slate-100">
                        ...
                        <p class="font-semibold text-slate-600">{{ __('dashboard.empty_sales_title') }}</p>
                        <p class="mt-1 max-w-xs text-sm text-slate-500">{{ __('dashboard.empty_sales_body') }}</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="bakery-card overflow-hidden">
            <div class="border-b border-slate-100 bg-gradient-to-br from-sky-50/80 to-white px-4 py-4 sm:px-5">
                <p class="text-xs font-bold uppercase tracking-wide text-sky-800/70">{{ __('dashboard.this_month') }}</p>
                <h2 class="text-base font-extrabold text-slate-900">{{ __('dashboard.cost_composition') }}</h2>
                ...
                @if ($hasCostChart)
                    <div class="relative mx-auto h-[200px] w-full max-w-[240px]">
                        <canvas
                            ...
                            aria-label="{{ __('dashboard.chart_cost_aria') }}"
                        ></canvas>
                    </div>
                    <p class="mt-3 text-center text-xs font-semibold text-slate-500">
                        {{ __('dashboard.total') }} {{ FormatHelper::rupiah($totalCostComposition) }}
                    </p>
                @else
    ...
    <div class="grid gap-4 lg:grid-cols-2 lg:items-stretch">
        <div class="bakery-card flex h-full min-h-0 flex-col overflow-hidden">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-gradient-to-r from-emerald-50/60 to-white px-4 py-3 sm:px-5">
                <div>
                    <p class="text-xs font-bold uppercase tracking-wide text-emerald-800/60">{{ __('dashboard.finance') }}</p>
                ...
                <a href="{{ route('admin.laba_rugi') }}" class="text-xs font-bold text-amber-700 hover:text-amber-800">
                    {{ __('dashboard.view_income_statement') }}
                </a>
            </div>
            ...
            <div class="mt-auto flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 bg-slate-50/80 px-4 py-3 text-xs sm:px-5">
                <span class="text-slate-500">{{ __('dashboard.net_margin') }}</span>
                <span class="font-extrabold tabular-nums {{ $profitMargin >= 0 ? 'text-emerald-700' : 'text-rose-600' }}">
                    {{ __('dashboard.net_margin_of_sales', ['pct' => $profitMargin]) }}
                </span>
            </div>
        </div>
        ...
        <div class="bakery-card flex h-full min-h-0 flex-col overflow-hidden">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-gradient-to-r from-sky-50/60 to-white px-4 py-3 sm:px-5">
                <div>
                    <p class="text-xs font-bold uppercase tracking-wide text-sky-800/60">{{ __('dashboard.operations') }}</p>
                    <h2 class="text-sm font-extrabold text-slate-900">{{ __('dashboard.production_summary') }}</h2>
                </div>
                <a href="{{ route('admin.produksi') }}" class="text-xs font-bold text-amber-700 hover:text-amber-800">
                    {{ __('dashboard.manage_production') }}
                </a>
            </div>
            ...
            @if ($totalProduction > 0)
                    ...
                    <div class="space-y-3">
                        <div class="grid grid-cols-2 gap-3">
                            <div class="rounded-xl bg-emerald-50/80 p-3 text-center ring-1 ring-emerald-100">
                                ...
                                <p class="mt-0.5 text-xs font-semibold text-emerald-800/70">{{ __('app.common.success') }}</p>
                            </div>
                            <div class="rounded-xl bg-rose-50/80 p-3 text-center ring-1 ring-rose-100">
                                ...
                                <p class="mt-0.5 text-xs font-semibold text-rose-800/70">{{ __('app.common.failed') }}</p>
                            </div>
                        </div>

                        <div>
                            <div class="mb-1.5 flex justify-between text-xs font-semibold text-slate-500">
                                <span>{{ __('dashboard.success_rate') }}</span>
                        ...
                        @if ($latestProduction)
                            <div class="rounded-xl bg-slate-50/90 px-3.5 py-3 ring-1 ring-slate-100">
                                <p class="text-[11px] font-bold uppercase tracking-wide text-slate-400">{{ __('dashboard.last_batch') }}</p>
                    ...
                    @if ($recentFailedProduction && $recentFailedProduction->id !== $latestProduction?->id)
                        <div class="mt-auto flex items-start gap-2 rounded-xl bg-rose-50 px-3 py-2.5 pt-3 ring-1 ring-rose-100">
                            ...
                            <p class="min-w-0 text-xs leading-relaxed text-rose-800">
                                {{ __('dashboard.failed_alert', ['product' => $recentFailedProduction->product_name]) }}
                            </p>
                        </div>
                    @endif
                </div>
                ...
                <div class="mt-auto flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 bg-slate-50/80 px-4 py-3 text-xs sm:px-5">
                    <span class="text-slate-500">{{ __('dashboard.batches_recorded', ['count' => $totalProduction]) }}</span>
                    <span class="font-extrabold tabular-nums text-sky-700">{{ __('dashboard.success_pct', ['pct' => $productionSuccessRate]) }}</span>
                </div>
            @else
                <div class="flex flex-1 flex-col items-center justify-center p-6 text-center">
                    <p class="text-sm font-semibold text-slate-600">{{ __('dashboard.empty_production_title') }}</p>
                    <p class="mt-1 text-xs text-slate-500">{{ __('dashboard.empty_production_body') }}</p>
                    <a href="{{ route('admin.produksi') }}" class="mt-3 inline-block text-xs font-bold text-amber-700 hover:text-amber-800">
                        {{ __('dashboard.open_production') }}
                    </a>
                </div>
            @endif
        </div>
    ...
    <div class="bakery-card overflow-hidden">
        <div class="border-b border-slate-100 bg-slate-50/80 px-4 py-3 sm:px-5">
            <p class="text-xs font-bold uppercase tracking-wide text-slate-400">{{ __('dashboard.system') }}</p>
            <h2 class="text-sm font-extrabold text-slate-900">{{ __('dashboard.activity_log') }}</h2>
        </div>
        <ul class="divide-y divide-slate-50 px-4 py-2 sm:px-5">
            ...
            @empty
                <li class="py-12 text-center text-sm text-slate-500">{{ __('dashboard.empty_activity') }}</li>
            @endforelse
        </ul>
    </div>
</div>
