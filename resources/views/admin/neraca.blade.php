...
@php
    ...
    $pageSubtitle = $filterLabel ?? 'Posisi keuangan';
...
<div class="space-y-4">
    <form method="GET" class="flex flex-wrap items-end gap-3">
        ...
        <div>
            <label for="as_of" class="mb-1 block text-xs font-bold text-slate-600">Per tanggal</label>
            <input
                type="date"
                id="as_of"
                name="as_of"
    ...
    <div class="rounded-2xl bg-gradient-to-br from-amber-50 to-white p-4 ring-1 ring-amber-200/50 sm:p-5">
        <p class="text-xs font-bold uppercase tracking-wide text-amber-900/70">Persamaan neraca</p>
        <div class="mt-3 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-center sm:gap-4">
            <div class="rounded-xl bg-white px-4 py-3 text-center shadow-sm ring-1 ring-sky-100 sm:min-w-[140px]">
                <p class="text-[11px] font-semibold text-sky-700">Aset</p>
            ...
            <div class="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:gap-3">
                <div class="rounded-xl bg-white px-4 py-3 text-center shadow-sm ring-1 ring-slate-100 sm:min-w-[130px]">
                    <p class="text-[11px] font-semibold text-slate-600">Liabilitas</p>
                ...
                <div class="rounded-xl bg-white px-4 py-3 text-center shadow-sm ring-1 ring-emerald-100 sm:min-w-[130px]">
                    <p class="text-[11px] font-semibold text-emerald-700">Ekuitas</p>
        ...
        <p class="mt-3 text-center text-xs {{ $isBalanced ? 'font-semibold text-emerald-700' : 'font-semibold text-rose-600' }}">
            @if ($isBalanced)
                ✓ Neraca seimbang pada tanggal ini
            @else
                Selisih {{ FormatHelper::rupiah($data['difference']) }} — periksa jurnal atau tanggal laporan
            @endif
        </p>
    </div>
    ...
    @if ($view === 'ringkasan')
        <div class="grid gap-4 lg:grid-cols-2">
            <div class="bakery-card p-4 sm:p-5">
                <div class="mb-4 flex items-center justify-between gap-2">
                    <p class="text-sm font-extrabold text-sky-800">Komposisi aset</p>
                    <p class="text-xs font-semibold text-slate-500">{{ $accountCount }} akun aktif</p>
                </div>
                ...
                <div class="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                    <span class="text-xs font-bold uppercase text-slate-500">Total aset</span>
                    <span class="text-lg font-extrabold tabular-nums text-sky-800">{{ FormatHelper::rupiah($data['total_assets']) }}</span>
                </div>
            </div>

            <div class="bakery-card p-4 sm:p-5">
                <p class="mb-4 text-sm font-extrabold text-slate-800">Komposisi liabilitas & ekuitas</p>
                @include('admin.partials.neraca-summary-bars', [
        ...
        <p class="text-xs text-slate-500">
            Tab <strong>Ringkasan</strong> menampilkan subtotal per kelompok. Buka tab <strong>Aset</strong> atau <strong>Liabilitas & Ekuitas</strong> untuk melihat rincian akun (klik baris untuk membuka).
        </p>
    @endif
    ...
    @if ($view === 'aset')
        <div class="bakery-card overflow-hidden">
            <div class="border-b border-slate-100 bg-sky-50/80 px-4 py-3 sm:px-5">
                <p class="font-extrabold text-sky-900">Detail aset</p>
                <p class="text-xs text-sky-800/70">Klik kelompok untuk melihat akun · {{ FormatHelper::dateId($asOf) }}</p>
            </div>
            <div class="space-y-2 p-3 sm:p-4">
                ...
                @empty
                    <div class="rounded-2xl bg-slate-50 px-6 py-12 text-center">
                        <p class="font-semibold text-slate-600">Tidak ada saldo aset</p>
                        <p class="mt-1 text-sm text-slate-500">Coba tanggal lain atau pastikan sudah ada jurnal.</p>
                    </div>
                @endforelse
            </div>
        ...
        <div class="space-y-4">
            <div class="bakery-card overflow-hidden">
                <div class="border-b border-slate-100 bg-slate-50 px-4 py-3 sm:px-5">
                    <p class="font-extrabold text-slate-800">Liabilitas</p>
                    <p class="text-xs text-slate-500">Kewajiban jangka pendek dan lainnya</p>
                </div>
                <div class="space-y-2 p-3 sm:p-4">
                    ...
                    @empty
                        <p class="rounded-xl bg-slate-50 px-4 py-8 text-center text-sm text-slate-400">Tidak ada saldo liabilitas</p>
                    @endforelse
                </div>
            ...
            <div class="bakery-card overflow-hidden">
                <div class="border-b border-slate-100 bg-emerald-50/80 px-4 py-3 sm:px-5">
                    <p class="font-extrabold text-emerald-900">Ekuitas</p>
                    <p class="text-xs text-emerald-800/70">Modal dan laba ditahan</p>
                </div>
                <div class="space-y-2 p-3 sm:p-4">
                    ...
                    @empty
                        <p class="rounded-xl bg-slate-50 px-4 py-8 text-center text-sm text-slate-400">Tidak ada saldo ekuitas</p>
                    @endforelse
                </div>
