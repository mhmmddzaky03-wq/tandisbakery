@php
    ...
    $hasilSub = $isBerhasil ? 'Hasil produksi tercatat' : 'Produksi tidak menghasilkan stok';

    if ($isBerhasil) {
        $stockContribValue = '+'.number_format($stockContribution, 0, ',', '.').' '.$record->satuan;
        $stockContribSub = $catalogProduct
            ? 'Stok katalog saat ini: '.number_format($catalogProduct->jumlah, 0, ',', '.').' '.$catalogProduct->satuan
            : 'Belum didaftarkan di katalog produk';
        $stockContribTone = $catalogProduct ? 'blue' : 'amber';
    } else {
        $stockContribValue = '—';
        $stockContribSub = 'Tidak menambah stok katalog';
        $stockContribTone = 'rose';
    }

    $materialSub = $materialCount.' bahan baku · '.$bahanDasarCount.' bahan dasar';
...
<div class="space-y-6">
    @if (! $isBerhasil)
        <div class="flex items-start gap-3 rounded-xl border border-rose-100 bg-rose-50/80 px-4 py-3 text-sm text-rose-800">
            ...
            <div>
                <p class="font-bold">Produksi gagal</p>
                <p class="mt-0.5 text-rose-700">Hasil produksi ini tidak menambah stok katalog produk, meskipun pemakaian bahan baku tetap tercatat.</p>
            </div>
        </div>
    @endif
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <x-kpi-card
            ...
            :value="$record->status"
            :sub="$isBerhasil ? 'Produksi berhasil' : 'Produksi gagal'"
            :tone="$statusTone"
    ...
    <div class="bakery-card">
        <div class="bakery-card-header border-b border-slate-100 pb-4">
            <h2 class="text-base font-extrabold text-slate-900">Informasi Produksi</h2>
        </div>
        <div class="bakery-card-body">
            <dl class="grid gap-x-8 gap-y-0 sm:grid-cols-2">
                <div class="flex items-center justify-between gap-4 border-b border-slate-100 py-3 text-sm">
                    <dt class="text-slate-400">ID Produksi</dt>
                ...
                <div class="flex items-start justify-between gap-4 border-b border-slate-100 py-3 text-sm sm:border-b-0">
                    <dt class="shrink-0 text-slate-400">Produk Katalog</dt>
                    <dd class="text-right font-semibold text-slate-800">
                        @if ($catalogProduct)
                            ...
                            <div class="mt-0.5 text-xs font-normal text-slate-500">
                                Stok {{ number_format($catalogProduct->jumlah, 0, ',', '.') }} {{ $catalogProduct->satuan }}
                            </div>
                        @else
        ...
        <div class="bakery-card">
            ...
            <div class="bakery-card-body overflow-x-auto pt-2">
                <table class="bakery-table w-full min-w-[48rem]">
                    <thead>
                        <tr>
                            <th class="w-[10rem]">Bahan Dasar</th>
                            <th class="min-w-[14rem]">Batch Adonan</th>
                    ...
                    <tfoot>
                        <tr class="bg-violet-50/60">
                            <td colspan="4" class="px-4 py-3 text-right text-sm font-bold text-violet-900">Total bahan dasar</td>
    ...
    <div class="bakery-card">
        <div class="bakery-card-header bakery-card-header--bordered">
            ...
            <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{{ $materialCount }} baris</span>
        </div>
        <div class="bakery-card-body overflow-x-auto pt-2">
            @if ($record->materialUsages->isNotEmpty())
                <table class="bakery-table w-full min-w-[48rem]">
                    <thead>
                        <tr>
                            <th class="w-[10rem]">Bahan Baku</th>
                    ...
                    <tfoot>
                        <tr class="bg-amber-50/60">
                            <td colspan="4" class="px-4 py-3 text-right text-sm font-bold text-amber-900">Total bahan baku</td>
                        ...
                        @if ($record->bahanDasarUsages->isNotEmpty())
                            <tr class="bg-violet-50/40">
                                <td colspan="4" class="px-4 py-3 text-right text-sm font-bold text-slate-700">Total keseluruhan bahan</td>
            ...
            @else
                <div class="py-10 text-center">
                    <p class="text-sm font-semibold text-slate-500">Belum ada pemakaian bahan baku tercatat.</p>
                </div>
            @endif
        </div>
    </div>
