...
<div>
    <div class="bakery-card" data-table-search>
        <div class="bakery-card-header bakery-card-header--bordered">
            <div class="bakery-card-header__title">{{ __('app.tables.base_materials_list') }}</div>
            <div class="bakery-card-header__actions">
            <x-table-search :placeholder="__('bahan_dasar.search')" :value="$search ?? ''" />
            </div>
        </div>
        <div class="bakery-card-body bakery-table-wrap pt-2">
            <table class="bakery-table">
                ...
                <tbody data-table-search-body>
                    @forelse ($items as $item)
                        ...
                        <tr data-searchable-row data-search="{{ strtolower($item->id.' '.$item->nama.' '.$satuan.' '.$statusLabel) }}">
                            ...
                            <td class="min-w-[10rem]">
                                ...
                                <div class="mt-0.5 text-[11px] font-semibold text-slate-400">{{ __('bahan_dasar.batch_count', ['count' => $item->batches_count]) }}</div>
                            </td>
                            ...
                            <td>
                                <div class="flex items-center justify-center gap-1">
                                    <a href="{{ route($showRoute, $item->id) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition hover:bg-sky-50 hover:text-sky-600" title="{{ __('app.common.detail') }}" aria-label="{{ __('app.common.detail') }}">
                                        <x-icons.info-circle class="h-4 w-4" />
                                    </a>
                                    <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-sky-600" data-modal-open="edit-bahan-dasar-{{ $item->id }}" title="{{ __('app.common.edit') }}" aria-label="{{ __('app.common.edit') }}">
                                        <x-icons.pencil />
                                    </button>
                                    <form method="POST" action="{{ route($destroyRoute, $item->id) }}" class="inline" onsubmit="return confirm('{{ __('bahan_dasar.confirm_delete') }}')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-500 transition hover:bg-rose-50 hover:text-rose-600" title="{{ __('app.common.delete') }}" aria-label="{{ __('app.common.delete') }}" @disabled(! $item->canBeDeleted())>
                                            <x-icons.trash />
                                        </button>
                                    </form>
                                </div>
                    ...
                    @empty
                        <tr data-table-empty>
                            <td colspan="5" class="px-4 py-12 text-center text-sm text-slate-500">{{ __('bahan_dasar.empty') }}</td>
                        </tr>
                    @endforelse
                    <tr data-table-no-results class="hidden">
                        <td colspan="5" class="px-4 py-12 text-center text-sm text-slate-500">{{ __('app.common.not_found') }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <x-modal id="bahan-dasar-baru" :title="__('bahan_dasar.modal_add')" :subtitle="__('bahan_dasar.modal_add_sub')" size="lg" :scrollable="true" :auto-open="$isCreateTarget">
        ...
        <x-slot:footer>
            <x-form-actions form="form-bahan-dasar-baru" compact :submit="__('bahan_dasar.submit_save')" />
        </x-slot:footer>
    </x-modal>
    ...
    @foreach ($items as $item)
        ...
        <x-modal id="edit-bahan-dasar-{{ $item->id }}" :title="__('bahan_dasar.modal_edit')" :subtitle="$item->id" size="md" :auto-open="$isEditTarget">
            <form method="POST" action="{{ route($updateRoute, $item->id) }}" class="space-y-3" data-modal-form>
                ...
                <div>
                    <label for="field-nama-bd-edit-{{ $item->id }}" class="mb-1.5 block text-xs font-bold text-slate-600">{{ __('bahan_dasar.field_name') }} <span class="text-rose-500">*</span></label>
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <div>
                        <label for="field-satuan-bd-edit-{{ $item->id }}" class="mb-1.5 block text-xs font-bold text-slate-600">{{ __('app.common.unit') }} <span class="text-rose-500">*</span></label>
                    ...
                    <div>
                        <label for="field-min-bd-edit-{{ $item->id }}" class="mb-1.5 block text-xs font-bold text-slate-600">{{ __('stock.field_min') }} <span class="text-rose-500">*</span></label>
