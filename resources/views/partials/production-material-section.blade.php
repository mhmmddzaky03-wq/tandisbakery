...
<div
    ...
    data-select-placeholder="{{ __('production.material_select') }}"
    data-batch-placeholder="{{ __('production.material_batch_select') }}"
...
>
    ...
    @if ($materials->isEmpty())
        <div class="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-4 py-8 text-center">
            <p class="text-sm font-semibold text-slate-600">{{ __('production.material_empty') }}</p>
        </div>
    @else
        <div class="overflow-hidden rounded-xl ring-1 ring-slate-200">
            <div class="hidden border-b border-slate-100 bg-slate-50/90 px-3 py-2.5 text-[11px] font-bold uppercase tracking-wide text-slate-500 sm:grid sm:grid-cols-[minmax(11rem,1fr)_8.5rem_9rem_2rem] sm:gap-2.5 sm:px-4">
                <span>{{ __('production.material_header') }}</span>
        ...
        <template data-production-row-template>
            <div class="group px-3 py-3 transition sm:grid sm:grid-cols-[minmax(11rem,1fr)_8.5rem_9rem_2rem] sm:items-center sm:gap-2.5 sm:px-4 sm:py-2.5" data-production-row>
                <div class="min-w-0 space-y-1.5">
                    <label class="mb-1 block text-[11px] font-bold text-slate-500 sm:sr-only">{{ __('production.label_raw_material') }}</label>
                    <select
                    ...
                    >
                        <option value="" disabled selected>{{ __('production.material_select') }}</option>
                    </select>
                    <select
                        class="bakery-input hidden h-9 w-full min-w-0 text-xs"
                        data-material-batch-select
                    >
                        <option value="" disabled selected>{{ __('production.material_batch_select') }}</option>
                    </select>
                    <input type="hidden" value="" data-material-batch-hidden disabled />
                </div>

                <div class="mt-2 sm:mt-0">
                    <label class="mb-1 block text-[11px] font-bold text-slate-500 sm:sr-only">{{ __('production.label_dose') }}</label>
                    <div class="unit-input-group h-10" data-material-qty-group>
                        ...
                        <div class="unit-input-group__units hidden" data-material-unit-wrap>
                            ...
                            <div class="unit-toggle unit-toggle--compact" data-unit-toggle data-material-unit-toggle role="group" aria-label="{{ __('app.common.unit') }}"></div>
                        </div>
                ...
                <div class="mt-2 sm:mt-0">
                    <span class="mb-1 block text-[11px] font-bold text-slate-400 sm:sr-only">{{ __('app.common.stock') }}</span>
                    <div class="unit-stock-card unit-stock-card--production">
                        <div class="unit-stock-card__qty-row">
                            ...
                            <div class="unit-stock-card__unit-slot">
                                <div class="hidden shrink-0" data-material-stock-unit-wrap>
                                    <div class="unit-toggle unit-toggle--compact" data-unit-toggle data-material-stock-unit-toggle role="group" aria-label="{{ __('app.common.unit') }}"></div>
                                </div>
                        ...
                        <div class="unit-stock-card__remain text-[10px] leading-tight text-slate-500">
                            {{ __('production.label_remaining') }} <span data-material-remain-qty class="font-bold tabular-nums text-emerald-600">—</span>
                        </div>
                    </div>
                </div>
